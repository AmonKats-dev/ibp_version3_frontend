...
const IntegrationsPbs = () => {
  const [links, setLinks] = useState([]);
  const [resources, setResources] = useState([]);
  const [selectedVote, setSelectedVote] = useState();

  const [projects, setProjects] = useState([]);
  const [projectsPBS, setProjectsPBS] = useState([]);
    ...
    if (selectedVote) {
      dataProvider
        .integrations("pbs", {
          ...
          sort_field: "id",
          filter: { vote_code: selectedVote.code, is_not_linked: true },
        })
        .then((res) => {
          if (res && res.data) {
            setProjectsPBS(res.data);
          }
        });
      ...
      dataProvider
        .getListOfAll("projects", {
          filter: {
            is_deleted: false,
            is_not_budgeted: true,
            organization_id: Number(selectedVote.id),
          },
        })
        .then((response) => {
  ...
  React.useEffect(() => {
    dataProvider
      .getListOfAll("organizations", { filter: { level: 1 } })
      .then((res) => {
        if (res && res.data) {
  ...
  const handleSubmit = () => {
    const resultLinks = links.map((link) => {
      ...
      const pbsProject = lodash.find(
        projectsPBS,
        (item) => item.id === link.checkedRight
      );
      return {
        pbs_project_id: pbsProject.id,
        project_coa_code: pbsProject.project_coa_code,
  ...
  const handleChange = (ev) => {
    ...
    setSelectedVote(ev.target.value);
  };

  return (
    <Card className={classes.root}>
      <h2 className={classes.rootTitle}>PBS Integration</h2>
      ...
      {selectedVote && projects.length > 0 && (
        <CardContent className={classes.content}>
          ...
          <div className={classes.listContent}>
            <h3 className={classes.title}>PBS</h3>
            <TextField
              fullWidth
              label="Search"
              variant="outlined"
            ...
            />
            <List className={classes.list}>
              {projectsPBS && rightSideProjects().length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <ListItemText
                        ...
                        primary={
                          project &&
                          `${project.project_coa_code} - ${project.project_name}`
                        }
                      />
                    </ListItem>
                  );
            ...
            <List className={classes.list}>
              {projects &&
                ...
                links.map((project) => {
                  ...
                  const listItem = `${projectLeft.code} - ${projectRight.project_coa_code}`;
...
export default IntegrationsPbs;
