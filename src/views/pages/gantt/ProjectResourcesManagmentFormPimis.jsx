...
// Lazy initialization of zoomConfig to avoid accessing ganttProjectChart at module load
function getZoomConfig() {
  const chart = initGanttProjectChart();
  if (!chart || !chart.date) {
    // Return a basic config if Gantt is not available
    return {
      levels: [
        ...
        scales: [
          {
            ...
            format: function (date) {
              var dateToStr = chart.date.date_to_str("%d %M");
              var endDate = chart.date.add(date, -6, "day");
              var weekNum = chart.date.date_to_str("%W")(date);
              return (
                "#" +
                weekNum +
                ", " +
      ...
      },
      {
        ...
        scales: [
          { unit: "month", step: 1, format: "%M" },
          {
            ...
            step: 1,
            format: function (date) {
              var dateToStr = chart.date.date_to_str("%M");
              var endDate = chart.date.add(
                chart.date.add(date, 3, "month"),
                -1,
                "day"
              );
