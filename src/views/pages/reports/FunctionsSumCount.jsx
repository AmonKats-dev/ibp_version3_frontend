...
function FunctionsSumCount(props) {
  ...
  React.useEffect(() => {
    ...
    dataProvider
      ...
      .then((response) => {
        if (response && response.data) {
          setData(response.data.filter((item) => item.function));
        }
      });
  }, []);

  const projectsGroup = getProjectsByFunctions(data);
  ...
  function getProjectsGroup() {
    if (selected) {
      const functionsData = lodash.groupBy(
        projectsGroup[selected],
        (item) => item && item.function && item.function.name
      );
              ...
              >
                <TableHead>
                  <TableRow className={classes.filledRow}>
                    <TableCell>
                      {appConfig.functions_config[1].name}
                      Name
                    </TableCell>
                    {phases.map((phase) => (
...
export default FunctionsSumCount;
