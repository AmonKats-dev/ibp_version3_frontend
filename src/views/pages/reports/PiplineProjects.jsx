...
function PiplineProjects(props) {
  ...
  const PIPELINE_PHASE_STEP = getFeatureValue("has_pipeline_report_step") || -1;
      ...
      .then((response) => {
        if (response && response.data) {
          const filteredData = response.data
            ...
            .filter(
              (item) =>
                item.phase?.sequence === PIPELINE_PHASE_ID &&
                item.workflow?.step === PIPELINE_PHASE_STEP
            );
  ...
  function getOrganizationString(projects, vote) {
    const currentOrg =
      projects[vote] && projects[vote][0]?.project_organization?.parent;
    return currentOrg ? `${currentOrg.code} - ${vote}` : null;
  }
            ...
            lodash.keys(projectsBySectors).map((sector) => {
              ...
              return (
                <div>
                  ...
                  >
                    <b>{getOrganizationString(projectsBySectors, sector)}</b>
                  </Typography>
                  {projectsByVotes &&
                          ...
                          <TableHead>
                            <TableRow className={classes.filledRow}>
                              ...
                              {checkFeature("has_pimis_fields") ? (
                                ...
                                <TableCell align="center">
                                  Date of inclusion in the pipeline.
                                </TableCell>
                              )}
                            </TableRow>
                          </TableHead>
                              ...
                              <TableRow>
                                ...
                                <TableCell align="center">
                                  {project.current_project_detail
                                    ...
                                    costSumFormatter(
                                      project.current_project_detail
                                        .investment_stats.total_cost
                                      // calculateCost(project.current_project_detail.investment_stats)
                                    )}
                                </TableCell>
                                <TableCell align="center">
                                  {project.current_project_detail &&
