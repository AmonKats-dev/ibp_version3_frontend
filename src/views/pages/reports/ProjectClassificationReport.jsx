...
function RankingReport(props) {
  ...
  function getFilteredByType() {
    return lodash
      ...
      .reverse()
      .filter((item) =>
        filters[1] ? item.project_organization.parent.id === filters[1] : true
      )
      .filter((item) =>
        filters[2] ? item.project_organization.id === filters[2] : true
      );
  }
  ...
  function getOrganizations(level) {
    ...
    return lodash.uniqBy(
      organizations.filter((item) => item.level === level),
      "id"
    );
  }
