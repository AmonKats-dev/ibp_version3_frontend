  ...
  return (
    <div className="landscapeSection">
      <div className="content-area">
        <h2>Table 1: Physical Monitoring selected period</h2>
        <TableContainer>
          <Table
            size="small"
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  <strong>{`${translate("printForm.result_matrix.annual")} ${
                    meReport &&
                    meReport.start_date &&
                    getFiscalYearValue(
                      moment(meReport.start_date, "YYYY-MM-DD")
                    ).name
                  }`}</strong>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <strong>{`${translate("printForm.result_matrix.actual")} ${
                    meReport &&
                    meReport.start_date &&
                    getFiscalYearValue(
                      moment(meReport.start_date, "YYYY-MM-DD")
                ...
                <TableCell>
                  <strong>{`${translate("printForm.result_matrix.risk")} ${
                    meReport &&
                    meReport.start_date &&
                    getFiscalYearValue(
                      moment(meReport.start_date, "YYYY-MM-DD")
              ...
              {record && record.indicators && record.indicators.length !== 0 ? (
                record.indicators.map((indicator, idx) => (
                  <TableRow>
                    ...
                    <TableCell>
                      {indicator.targets[Number(meReport.year)]}
                    </TableCell>
                  ...
                  item.indicators && item.indicators.length !== 0 ? (
                    item.indicators.map((indicator, idx) => (
                      <TableRow>
                        ...
                        <TableCell>{indicator.actual}</TableCell>
                        <TableCell>{indicator.risk}</TableCell>
              ...
              {meReport &&
                meReport.me_outputs &&
                meReport.me_outputs.map((item, outputIdx) =>
                  item.indicators && item.indicators.length !== 0 ? (
                    item.indicators.map((indicator, idx) => (
                      <TableRow>
                        {idx === 0 ? (
